...
const Geolocation = () => {
  ...
  useEffect(() => {
    ...
    const success = (pos) => {
      ...
      const urlApi = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${API_key}`;
      apiRequest(urlApi);
      ...
      {isLoading ? (
        ...
        <>
          <video
            ...
            loop={true}
            muted={true}
            src={`./src/assets/videos/${information?.weather?.[0]?.icon}.mp4`}
          ></video>
          <div className="Container">
            ...
            <p className="gradesNumber">
              {isKelvin
                ? Number.parseFloat(information?.main?.temp - 272.15).toFixed(1)
                : Number.parseFloat(information?.main?.temp).toFixed(1)}{" "}
              <span> {isKelvin ? "°C" : "°K"}</span>{" "}
            </p>
            <img
          ...
          <div className="infoSecondary">
            ...
            <button type="" onClick={() => setIsKelvin(!isKelvin)}>
              {" "}
              Change to {isKelvin ? "Kelvin" : "Celsius"}
            </button>
          </div>
        </>
      )}
